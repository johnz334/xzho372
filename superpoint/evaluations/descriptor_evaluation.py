...
def homography_estimation(exper_name, keep_k_points=1000,
    ...
    correctness = []
    no = ['/home/xiaoran/桌面/SuperPoint-master/EXPERIMENT/outputs/superpoint_hpatches-v/119.npz','/home/xiaoran/桌面/SuperPoint-master/EXPERIMENT/outputs/superpoint_hpatches-v/3.npz']
    for path in paths:
        if path not in no:
            data = np.load(path)
